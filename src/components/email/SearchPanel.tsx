...
const SearchPanel: React.FC<SearchPanelProps> = ({
  ...
  const [shouldRetriggerSearch, setShouldRetriggerSearch] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (
        ...
        !searchModeRef.current.contains(e.target as Node)
      ) {
  ...
  // Re-trigger search when semantic toggle changes (if there's a query)
  useEffect(() => {
    if (shouldRetriggerSearch && searchRequest.body?.trim()) {
  ...
  const handleSearch = useCallback(
    async (overrideRequest?: SearchRequest) => {
      ...
      const hasValue = Object.values(requestToUse).some(
        (v) => v !== undefined && v !== "" && v !== false,
      );
      const hasQueryForSemantic =
        useSemanticSearch && (requestToUse.body || "").trim().length > 0;

      if (!hasValue && !hasQueryForSemantic) return;
      ...
      try {
        ...
        if (useSemanticSearch && (requestToUse.body || "").trim().length > 0) {
          // Use semantic search for conceptual relevance
          results = await gmailService.semanticSearch(
            requestToUse.body!,
            20,
          ...
          results = await gmailService.search(
            requestToUse,
            abortControllerRef.current.signal,
          );
        }
  ...
  const handleClear = useCallback(() => {
    ...
    const hasActiveFilters = Object.entries(newRequest).some(
      ([key, value]) =>
        key !== "body" &&
        value !== undefined &&
        value !== "" &&
  ...
  const activeFiltersCount = Object.entries(searchRequest).filter(
    ([key, value]) =>
      key !== "body" && value !== undefined && value !== "" && value !== false,
  ).length;
      ...
      >
        ...
        <div className="relative mr-1" ref={searchModeRef}>
          ...
          >
            ...
            <ChevronDown
              size={16}
              className={`transition-transform duration-200 ${showSearchModeMenu ? "rotate-180" : ""}`}
            />
          </button>
            ...
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl border border-gray-200 z-50 overflow-hidden">
              <div className="py-1">
                ...
                <button
                  ...
                  className="w-full px-4 py-3 hover:bg-purple-50 flex items-start gap-3 transition-colors border-t border-gray-100"
                >
                  <div className="flex-shrink-0 mt-0.5">
                    <Sparkles
                      size={20}
                      className={
                        searchRequest.useFuzzySearch && !useSemanticSearch
                          ? "text-purple-600"
                          : "text-gray-400"
                      }
                    />
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Tolerant to typos (e.g., "recieve" → "receive")
                    </p>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="w-full px-4 py-3 hover:bg-gradient-to-r hover:from-purple-50 hover:to-pink-50 flex items-start gap-3 transition-colors border-t border-gray-100"
                >
                  <div className="flex-shrink-0 mt-0.5">
                    <Brain
                      size={20}
                      className={
                        useSemanticSearch ? "text-purple-600" : "text-gray-400"
                      }
                    />
                  </div>
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      🤖 Finds related concepts (e.g., "money" → "invoice",
                      "payment")
                    </p>
                  </div>
                </button>
          ...
          <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-gray-100">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  updateField("hasAttachment", e.target.checked || undefined)
                }
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  updateField("isUnread", e.target.checked || undefined)
                }
                className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  updateField("isStarred", e.target.checked || undefined)
                }
                className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  updateField("isImportant", e.target.checked || undefined)
                }
                className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
